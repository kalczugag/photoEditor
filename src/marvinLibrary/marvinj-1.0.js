    ...
    for (var y = centerPixel_Y; y < centerPixel_Y + this.radius * 2; y++) {
        for (var x = centerPixel_X; x < centerPixel_X + this.radius * 2; x++) {
            if (
                ...
                this.resultMatrix[x - this.radius][y - this.radius][
                    this.GREEN
                ] +=
...
Moravec.prototype.process = function (
    ...
    for (var x = 0; x < cornernessMap.length; x++) {
        for (var y = 0; y < cornernessMap[x].length; y++) {
            cornernessMapOut[x][y] = this.nonmax(
                x,
                y,
                matrixSize,
                cornernessMap
...
FindTextRegions.prototype.process = function (
    ...
    for (var y = 0; y < imageIn.getHeight(); y++) {
        ...
        for (var i in list) {
            var seg = list[i];
            marvinSegments.push(
                new MarvinSegment(seg[0], seg[1], seg[2], seg[3])
            );
        }
    }
    ...
    if (attributesOut != null) {
        ...
        switch (returnType) {
            ...
            case "MarvinBlobSegment":
                attributesOut.set(
                    "blobSegments",
                    blobSegments(fillBuffer, segments)
                );
                break;
        }
    }
...
var marvinLoadPluginMethods = function (callback) {
    ...
    Marvin.averageColor = function (imageIn) {
        ...
        Marvin.plugins.averageColor.process(
            imageIn,
            null,
            attrOut,
            MarvinImageMask.NULL_MASK,
    ...
    Marvin.brightnessAndContrast = function (
        ...
        Marvin.plugins.brightnessAndContrast.setAttribute(
            "brightness",
            brightness
        );
    ...
    Marvin.combineByAlpha = function (imageIn, imageOther, imageOut, x, y) {
        ...
        Marvin.plugins.combineByAlpha.process(
            imageIn,
            imageOut,
            null,
            MarvinImageMask.NULL_MASK,
    ...
    Marvin.findTextRegions = function (
        imageIn,
        maxWhiteSpace,
        maxFontLineWidth,
        minTextWidth,
        ...
        Marvin.plugins.findTextRegions.setAttribute(
            "maxWhiteSpace",
            Marvin.getValue(maxWhiteSpace, 10)
        );
        Marvin.plugins.findTextRegions.setAttribute(
            "maxFontLineWidth",
            Marvin.getValue(maxFontLineWidth, 10)
        );
        Marvin.plugins.findTextRegions.setAttribute(
            "minTextWidth",
            Marvin.getValue(minTextWidth, 30)
        );
        Marvin.plugins.findTextRegions.setAttribute(
            "grayScaleThreshold",
            Marvin.getValue(grayScaleThreshold, 127)
        );
        Marvin.plugins.findTextRegions.process(
            imageIn,
            null,
            attrOut,
            MarvinImageMask.NULL_MASK,
    ...
    Marvin.iteratedFunctionSystem = function (
        imageIn,
        imageOut,
        rules,
        iterations
        ...
        Marvin.plugins.iteratedFunctionSystem.setAttribute(
            "iterations",
            iterations
        );
        Marvin.plugins.iteratedFunctionSystem.process(
            imageIn,
            imageOut,
            null,
            MarvinImageMask.NULL_MASK,
    ...
    Marvin.thresholding = function (
        ...
        Marvin.plugins.thresholding.setAttribute(
            "thresholdRange",
            thresholdRange
        );
