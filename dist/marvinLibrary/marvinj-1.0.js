...
MarvinColorModelConverter.rgbToBinary = function (img, threshold) {
    ...
    for (var y = 0; y < img.getHeight(); y++) {
        for (var x = 0; x < img.getWidth(); x++) {
            var gray = Math.ceil(img.getIntComponent0(x, y) * 0.3 +
...
DetermineFixedCameraBackground.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    if (!this.initialized) {
...
DetermineSceneBackground.prototype.getBackgroundPixel = function (x, y, images, threshold) {
    ...
    for (var i in images) {
        ...
        if (colors.length == 0) {
            ...
            for (var j in colors) {
                var c2 = colors[j];
                if (Math.abs(c2[0] - c[0]) < threshold * 0.3 &&
...
GaussianBlur.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    ...
    for (var x = 0; x < l_imageWidth; x++) {
        for (var y = 0; y < l_imageHeight; y++) {
            ...
            this.resultMatrix[x][y][this.RED] =
                (this.resultMatrix[x][y][0] / this.appiledkernelMatrix[x][y]) %
                    256;
            this.resultMatrix[x][y][this.GREEN] =
                (this.resultMatrix[x][y][1] / this.appiledkernelMatrix[x][y]) %
                    256;
            this.resultMatrix[x][y][this.BLUE] =
                (this.resultMatrix[x][y][2] / this.appiledkernelMatrix[x][y]) %
                    256;
...
AlphaBoundary.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
AverageColor.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    var totalR = 0;
    var totalG = 0;
    var totalB = 0;
    for (var x = 0; x < imageIn.getWidth(); x++) {
...
BlackAndWhite.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
BrightnessAndContrast.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    var r, g, b;
...
ColorChannel.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
Emboss.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    var l_arrMask = mask.getMask();
    for (var x = 0; x < imageIn.getWidth(); x++) {
        for (var y = 0; y < imageIn.getHeight(); y++) {
            if (l_arrMask != null && !l_arrMask[x][y]) {
...
GrayScale.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    // Mask
    var l_arrMask;
    if (mask != null) {
...
Invert.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    var l_arrMask = mask.getMask();
    var r, g, b;
    for (var x = 0; x < imageIn.getWidth(); x++) {
        for (var y = 0; y < imageIn.getHeight(); y++) {
...
Sepia.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
Thresholding.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
Thresholding.prototype.checkNeighbors = function (x, y, neighborhoodX, neighborhoodY, img) {
    ...
    for (var i = 0 - neighborhoodX; i <= neighborhoodX; i++) {
        for (var j = 0 - neighborhoodY; j <= neighborhoodY; j++) {
            ...
            if (color < getSafeColor(x + i, y + j, img) - range &&
...
ThresholdingNeighborhood.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
CombineByAlpha.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
...
Convolution.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
        ...
        for (var j = 0; j < matrix[0].length; j++) {
            if (matrix[i][j] != 0) {
                ...
                if (nx >= 0 &&
                    ...
                    resultRed +=
                        matrix[i][j] * imageIn.getIntComponent0(nx, ny);
                    resultGreen +=
                        matrix[i][j] * imageIn.getIntComponent1(nx, ny);
                    resultBlue +=
                        matrix[i][j] * imageIn.getIntComponent2(nx, ny);
                }
            }
        }
    }
...
Moravec.prototype.process = function (imageIn, imageOut, attrOut, mask, previewMode) {
...
Moravec.prototype.c = function (x, y, matrixSize, image) {
    ...
    if (x - (s + 1) >= 0 &&
        ...
        for (var d = 0; d < Moravec.directions.length; d++) {
            ...
            for (var i = -s; i <= s; i++) {
                for (var j = -s; j <= s; j++) {
                    temp += Math.pow(image.getIntComponent0(x + i, y + j) -
                        image.getIntComponent0(x + i + Moravec.directions[d][0], y + j + Moravec.directions[d][1]), 2);
                }
            }
            if (ret == -1 || temp < ret) {
                ret = temp;
...
Prewitt.prototype.process = function (imageIn, imageOut, attrOut, mask, previewMode) {
...
BoundaryFill.prototype.process = function (imgIn, imgOut, attributesOut, mask, previewMode) {
    ...
    while (l_list.length > 0) {
        ...
        while (true) {
            if (l_pointW.x - 1 >= 0 &&
                this.match(imgIn, l_pointW.x - 1, l_pointW.y, targetRed, targetGreen, targetBlue, this.threshold) &&
        ...
        // east
        while (true) {
            if (l_pointE.x + 1 < imgIn.getWidth() &&
                this.match(imgIn, l_pointE.x + 1, l_pointE.y, targetRed, targetGreen, targetBlue, this.threshold) &&
        ...
        for (var l_px = l_pointW.x; l_px <= l_pointE.x; l_px++) {
            ...
            if (l_point.y - 1 >= 0 &&
                this.match(imgIn, l_px, l_point.y - 1, targetRed, targetGreen, targetBlue, this.threshold) &&
                !fillMask[l_px][l_point.y - 1]) {
                l_list.push(new MarvinPoint(l_px, l_point.y - 1));
            }
            if (l_point.y + 1 < imgOut.getHeight() &&
                this.match(imgIn, l_px, l_point.y + 1, targetRed, targetGreen, targetBlue, this.threshold) &&
                !fillMask[l_px][l_point.y + 1]) {
                l_list.push(new MarvinPoint(l_px, l_point.y + 1));
            }
...
BoundaryFill.prototype.match = function (image, x, y, targetRed, targetGreen, targetBlue, threshold) {
    var diff = Math.abs(image.getIntComponent0(x, y) - targetRed) +
...
ErrorDiffusion.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    var color;
    var dif;
...
Closing.prototype.process = function (imgIn, imgOut, attributesOut, mask, previewMode) {
    var matrix = this.getAttribute("matrix");
...
Dilation.prototype.process = function (imgIn, imgOut, attributesOut, mask, previewMode) {
    var matrix = this.getAttribute("matrix");
    if (imgIn.getColorModel() == MarvinImage.COLOR_MODEL_BINARY &&
        matrix != null) {
        MarvinImage.copyColorArray(imgIn, imgOut);
...
Erosion.prototype.process = function (imgIn, imgOut, attributesOut, mask, previewMode) {
    var matrix = this.getAttribute("matrix");
    if (imgIn.getColorModel() == MarvinImage.COLOR_MODEL_BINARY &&
        matrix != null) {
        MarvinImage.copyColorArray(imgIn, imgOut);
...
FindTextRegions.prototype.process = function (imageIn, imageOut, attrOut, mask, previewMode) {
...
/*
    ...
    */
IteratedFunctionSystem.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    this.loadRules();
...
Crop.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    var x = this.getAttribute("x");
    var y = this.getAttribute("y");
...
FloodfillSegmentation.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    if (attributesOut != null) {
...
Scale.prototype.process = function (imageIn, imageOut, attributesOut, mask, previewMode) {
    if (!previewMode) {
        var width = imageIn.getWidth();
        var height = imageIn.getHeight();
        var newWidth = this.getAttribute("newWidth");
        var newHeight = this.getAttribute("newHeight");
        if (imageOut.getWidth() != newWidth ||
